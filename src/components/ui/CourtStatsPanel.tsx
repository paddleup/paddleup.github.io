...
const CourtStatsPanel: React.FC<CourtStatsPanelProps> = ({
  ...
  const completionPercentage = totalGamesForCourt > 0 ? Math.round((completedGamesForCourt / totalGamesForCourt) * 100) : 0;
  ...
  const sortedPlayerStats = [...playerStats].sort((a, b) => {
    // Sort by wins, then point differential, then seed
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-surface-alt/50 border-b border-border/30">
                <tr>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-text-main uppercase tracking-wider">Rank</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-text-main uppercase tracking-wider">Player</th>
                  <th className="px-3 py-2 text-center text-xs font-semibold text-text-main uppercase tracking-wider">W-L</th>
                  <th className="px-3 py-2 text-center text-xs font-semibold text-text-main uppercase tracking-wider">Win %</th>
                  <th className="px-3 py-2 text-center text-xs font-semibold text-text-main uppercase tracking-wider">Diff</th>
                  <th className="px-3 py-2 text-center text-xs font-semibold text-text-main uppercase tracking-wider">Games</th>
                </tr>
              </thead>
                ...
                {sortedPlayerStats.map((playerStats, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 whitespace-nowrap text-center">
                        <span className="text-sm font-medium text-text-main">
                          {(playerStats.wins + playerStats.losses)}/{sortedPlayerStats.length === 5 ? 4 : totalGamesForCourt}
                        </span>
                      </td>
                    </tr>
                  );
