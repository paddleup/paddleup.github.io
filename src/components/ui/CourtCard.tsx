...
const CourtCard: React.FC<CourtCardProps> = ({
  ...
  const completedGamesForCourt = court.games.filter(
    (g: any) => g.roundNumber === roundNumber && g.team1Score !== undefined && g.team2Score !== undefined
  ).length;
  ...
  const isEditable =
    (roundNumber === 1 && (typeof (window as any).areRound1UpdatesAllowed === "boolean"
      ? (window as any).areRound1UpdatesAllowed
      : true)) ||
    (roundNumber === 2 && (typeof (window as any).areRound2UpdatesAllowed === "boolean"
          ...
          {court.games.map((game: any, idx: number) => {
            ...
            return (
              <div key={game.id}>
                ...
                <div
                  className="bg-surface rounded-xl border border-border/30 shadow transition-all duration-200 overflow-hidden"
                >
                  <div className="p-2 space-y-1">
                    <div className={`rounded-lg transition-all duration-200 px-4 ${team1Highlight}`}>
                      <TeamRow
                        color="primary"
                    ...
                    <div className={`rounded-lg transition-all duration-200 px-4 ${team2Highlight}`}>
                      <TeamRow
                        color="success"
